...
**v1.8.4 前**は、デスクトップで初めて使用する際に組込みブラウザーの初期化が必要で、ブラウザーコア **約 400MB** のダウンロードおよび解凍が行われます。
**v1.8.4 以降**は、デスクトップ版から `kcef` 依存を削除したため、組込みブラウザーの初回使用時に約300MBのChromiumコアを**ダウンロードする必要はありません**。
...
2. ブラウザーの読み込みを待機し、Pixiv ログインページを表示します。（v1.8.4 を使用している場合、このステップはすぐ終わります。ここで止まる場合は、ログを添えて Issue ページへお越しください。）
